    ...
    public class IndexModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostIndexDocumentAsync(Guid id)
        {
            try
            {
                ...
                {
                    TempData["ErrorMessage"] = "B?n không có quy?n l?p ch? m?c tài li?u này vì b?n không ph?i là Tru?ng b? môn ph? trách.";
                    return RedirectToPage(new { courseId = document.CourseId });
                }

                await _documentService.IndexDocumentAsync(id, null);
                TempData["SuccessMessage"] = $"Ðã b?t d?u l?p ch? m?c tài li?u '{document.FileName}'.";
            }
            catch (Exception ex)
            {
                TempData["ErrorMessage"] = $"L?i l?p ch? m?c: {ex.Message}";
            }
