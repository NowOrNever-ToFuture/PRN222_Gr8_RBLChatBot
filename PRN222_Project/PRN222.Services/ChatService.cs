...
Chỉ trả về MÃ MÔN HỌC duy nhất (ví dụ: PRN222), không giải thích thêm. Nếu không xác định được, trả về ""UNKNOWN"".";
        ...
        public async Task<RagResponse> GenerateRagResponseAsync(string query, Guid? selectedCourseId = null)
        {
            ...
            // Bước 0b: Phát hiện meta-instruction (thay đổi ngôn ngữ/format) — tách ra và inject vào prompt
            string? metaInstruction = ExtractMetaInstruction(query);
            string actualQuery = metaInstruction != null ? StripMetaInstruction(query) : query;
            // Nếu sau khi strip còn quá ít ký tự, vẫn dùng query gốc nhưng thêm instruction
            if (string.IsNullOrWhiteSpace(actualQuery) || actualQuery.Length < 5)
                actualQuery = query;
            ...
            // Nếu có meta-instruction (ví dụ: trả lời bằng tiếng Anh), inject vào cuối prompt
            string metaInstructionSection = metaInstruction != null
                ? $"\n\n=== YÊU CẦU THÊM TỪ HỌC VIÊN ===\n{metaInstruction}"
                : "";
...
{actualQuery}{metaInstructionSection}

Hãy phản hồi một cách tự nhiên, mạch lạc, dễ hiểu và tràn đầy tinh thần hỗ trợ học thuật. Nếu học viên yêu cầu trả lời bằng ngôn ngữ khác, hãy thực hiện yêu cầu đó.";
        ...
        // Các keyword chỉ thay đổi cách trả lời (ngôn ngữ, format) — KHÔNG phải câu hỏi học thuật
        private static readonly string[] MetaInstructionKeywords = {
            "trả lời bằng tiếng anh", "answer in english", "reply in english", "respond in english",
            "trả lời bằng tiếng việt", "answer in vietnamese",
            "hãy trả lời bằng", "please answer in", "trả lời ngắn gọn", "trả lời chi tiết",
            "answer briefly", "answer in detail", "reply briefly",
            "viết bằng tiếng anh", "write in english", "summarize in english",
            "tóm tắt bằng tiếng anh", "giải thích bằng tiếng anh",
            "trả lời theo dạng bảng", "trả lời dạng danh sách", "liệt kê"
        };

        /// <summary>
        /// Phát hiện câu query có chứa meta-instruction (thay đổi ngôn ngữ/format trả lời).
        /// Trả về instruction string nếu tìm thấy, null nếu không.
        /// </summary>
        private static string? ExtractMetaInstruction(string query)
        {
            string lower = query.ToLowerInvariant();
            foreach (var keyword in MetaInstructionKeywords)
            {
                if (lower.Contains(keyword))
                    return keyword; // trả về keyword tìm thấy để inject vào prompt
            }
            return null;
        }

        /// <summary>
        /// Bóc tách phần meta-instruction ra khỏi query gốc để lấy phần nội dung học thuật thực.
        /// </summary>
        private static string StripMetaInstruction(string query)
        {
            string lower = query.ToLowerInvariant();
            foreach (var keyword in MetaInstructionKeywords)
            {
                int idx = lower.IndexOf(keyword, StringComparison.OrdinalIgnoreCase);
                if (idx >= 0)
                {
                    // Loại bỏ phần keyword, lấy phần còn lại
                    string before = query.Substring(0, idx).Trim().TrimEnd(',', '.', '!', '?', ';', '-');
                    string after = query.Substring(idx + keyword.Length).Trim().TrimStart(',', '.', '!', '?', ';', '-');
