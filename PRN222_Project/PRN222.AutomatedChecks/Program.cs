...

{
...

    {
        ...
        [
            ...
            ("CourseService auto-assigns head lecturer to teaching list", TestHeadLecturerAutoAssignedToCourseAsync),
...

    {
        ...
        db.CourseLecturers.AddRange(

            new CourseLecturer { Id = Guid.NewGuid(), CourseId = course1.Id, LecturerId = lecturer.Id },

            new CourseLecturer { Id = Guid.NewGuid(), CourseId = course2.Id, LecturerId = lecturer.Id });
        ...
        AssertTrue((await db.CourseLecturers.Where(cl => cl.LecturerId == lecturer.Id).CountAsync()) == 0, "Teaching assignments should be cleared when leaving lecturer role.");
...

    {
...

        {
            ...
            AssertTrue(true, "Expected transfer-first validation message.");
    ...
    private async Task TestHeadLecturerAutoAssignedToCourseAsync()

    {
        ...
        AssertTrue(await db.CourseLecturers.AnyAsync(cl => cl.CourseId == course.Id && cl.LecturerId == lecturer.Id), "Head lecturer should also be inserted into CourseLecturers.");
...

    {
        ...
        db.CourseLecturers.AddRange(
            ...
            new CourseLecturer { Id = Guid.NewGuid(), CourseId = course.Id, LecturerId = head.Id },

            new CourseLecturer { Id = Guid.NewGuid(), CourseId = course.Id, LecturerId = other.Id });
...

        {
            ...
            AssertTrue(true, "Expected non-head lecturer rejection.");
...

    {
        ...
        db.CourseLecturers.Add(new CourseLecturer { Id = Guid.NewGuid(), CourseId = course.Id, LecturerId = head.Id });
...

    {
        ...
        var outsider = new User { Id = Guid.NewGuid(), Username = "outsider", FullName = "Outsider", PasswordHash = "hash", Role = "Lecturer" };
        ...
        db.Users.AddRange(head, lecturer, outsider, student);
        ...
        db.CourseLecturers.AddRange(
            ...
            new CourseLecturer { Id = Guid.NewGuid(), CourseId = course.Id, LecturerId = lecturer.Id });
        ...
        var outsiderDocs = await service.GetDocumentsAsync(outsider.Id, "Lecturer", course.Id);
